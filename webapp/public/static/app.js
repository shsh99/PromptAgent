...
//   new      → 새 기능 추가          (파란 뱃지)
//   improve  → 기존 기능 개선        (초록 뱃지)
//   fix      → 버그·오류 수정        (빨간 뱃지)
//   refactor → 내부 구조 정리        (회색 뱃지)
// version: 'vX.Y.Z' 형태로 작성 (선택)
// ─────────────────────────────────────────────────────────────────
...
function showUpdates() {
  ...
  const latestVersion = CHANGELOG_ITEMS[0]?.version || '';

  content.innerHTML = CHANGELOG_ITEMS.map((item, index) => {
    const isLatest = index === 0;
    const tagMeta = CHANGELOG_TAG_META;

    const itemsHtml = item.items.map(entry => {
      const meta = tagMeta[entry.tag] || tagMeta.new;
      return `
