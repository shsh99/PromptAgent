...
function syncHeaderToggleState() {
  document.querySelectorAll('[data-theme-switch]').forEach((button) => {
    ...
    button.setAttribute('title', active ? '다크 모드로 전환' : '라이트 모드로 전환')
    button.setAttribute('aria-label', active ? '다크 모드로 전환' : '라이트 모드로 전환')
  })
  document.querySelectorAll('[data-lang-switch]').forEach((button) => {
    const active = state.promptLanguage === 'en'
    button.setAttribute('aria-pressed', String(active))
    button.dataset.active = active ? 'true' : 'false'
    button.setAttribute('title', active ? '영어로 전환' : '한국어로 전환')
    button.setAttribute('aria-label', active ? '영어로 전환' : '한국어로 전환')
  })
